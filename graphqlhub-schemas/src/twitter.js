...
let TweetType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    retweets: {
      ...
      resolve: ({ id_str: tweetId }, { limit }) =>
        twitter.getRetweets(tweetId, limit)
    }
  })
});
  ...
  parseLiteral: ast => {
    if (ast.kind !== Kind.STRING && ast.kind !== Kind.INT) {
      throw new GraphQLError(
        "Query error: Can only parse Integer and String but got a: " + ast.kind,
        [ast]
      );
    }
...
let twitterType = new GraphQLObjectType({
  ...
  fields: {
    user: {
      ...
      resolve: (_, { identifier, identity }) =>
        twitter.getUser(identifier, identity)
    },
    tweet: {
      type: TweetType,
      args: {
    ...
    search: {
      ...
      args: {
        ...
        count: {
          type: GraphQLInt,
          description:
            "The number of tweets to return per page, up to a maximum of 100. This was formerly the “rpp” parameter in the old Search API."
        },
        result_type: {
