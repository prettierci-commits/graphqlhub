...
let CDN = {
  ...
  graphiqlCss:
    "https://cdnjs.cloudflare.com/ajax/libs/graphiql/0.6.3/graphiql.min.css",
  graphiqlJs:
    "https://cdnjs.cloudflare.com/ajax/libs/graphiql/0.6.3/graphiql.min.js",
  normalizeCss:
...
let HTMLS = {
  PLAYGROUND: () =>
    compileFile(
      path.join(__dirname, "..", "public", "playground", "index.html")
    ),
...
app.use(
  "/graphql",
  cors(),
  instrumentationMiddleware(Schema, timingCallback, { addToResponse: false }),
  graphqlHTTP((req, res) => {
    return {
      schema: Schema,
